...
const keyfilename = path + "/keys/hhpriv.pem";
...
let bunq = new Bunq(INSTALLATION_TOKEN, API_TOKEN, privateKey);
let userId;

bunq.initSession().then(function() {
    bunq.getUser().then(function(respGetUser) {
        userId = bunq.parseResponse(respGetUser)[0]["UserPerson"]["id"];
        console.log(respGetUser);
        bunq.getMonetaryAccount(userId).then(function(respMonAcc) {
            ...
            bunq.getTransactions(userId, firstAccount.id).then(function(respTransactions) {
